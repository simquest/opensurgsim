...
class MockOsgCamera : public SurgSim::Graphics::OsgCamera
{
public:
	/// Constructor
	...
	explicit MockOsgCamera(const std::string& name) : SurgSim::Graphics::OsgCamera(name), SurgSim::Graphics::Actor(name),
		m_numUpdates(0),
		m_sumDt(0.0)
	{
	}
	...
	/// Returns the OSG switch node that controls the camera visibility
	osg::ref_ptr<osg::Switch> getOsgSwitch() const
	{
		return static_cast<osg::Switch*>(getOsgNode().get());
	}
...
class MockOsgView : public SurgSim::Graphics::OsgView
{
public:
	/// Constructor
	...
	/// \post m_numUpdates and m_sumDt are initialized to 0
	/// \post m_transform is set to identity
	explicit MockOsgView(const std::string& name) : SurgSim::Graphics::OsgView(name),
		m_numUpdates(0),
		m_sumDt(0.0),
		m_isInitialized(false),
		m_isAwoken(false)
