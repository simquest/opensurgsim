...
{
	viewElement->getCamera()->setLocalPose(makeRigidTranslation(Math::Vector3d(0.0, 0.0, 6.0)));
	...
	graphics->loadMesh("Geometry/Cube_with_texture.ply");
	...
	material->addUniform("sampler2D", "stainTexture");
	material->setValue("stainTexture", texture);
	...
	auto stain = std::make_shared<Graphics::DecalBehavior>("Decals");
	stain->setPainter(collisionRepresentation);
	//stain->setTexture(texture);
	element->addComponent(stain);
