...
TEST(OsgPlaneRepresentationRenderTests, MovingPlanesTest)
{
	...
	for (int i = 0; i < numSteps; ++i)
	{
		...
		/// Interpolate pose
		planeRepresentation1->setCurrentPose(makeRigidTransform(makeRotationQuaternion((1.0 - t) * startAngle1 + t * endAngle1,
			Vector3d(1.0, 0.0, 0.0)), (1.0 - t) * startPosition1 + t * endPosition1));
		planeRepresentation2->setCurrentPose(makeRigidTransform(makeRotationQuaternion((1.0 - t) * startAngle2 + t * endAngle2,
