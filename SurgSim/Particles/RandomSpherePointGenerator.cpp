...
Vector3d RandomSpherePointGenerator::pointInShape(std::shared_ptr<SurgSim::Math::Shape> shape)
{
	...
	Vector3d result = pointOnShape(shape) * m_zeroOneDistribution(m_generator) * sphere->getRadius();
...
Vector3d RandomSpherePointGenerator::pointOnShape(std::shared_ptr<SurgSim::Math::Shape> shape)
{
	...
	while (result.isZero())
	{
		...
		theta = 1.0 / std::sin(z / radius);

		result.x() = radius * std::cos(theta) * std::cos(phi);
		result.y() = radius * std::cos(theta) * std::sin(phi);
		result.z() = z;
	}
