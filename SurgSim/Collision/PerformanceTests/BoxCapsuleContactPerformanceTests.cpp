...
TEST(BoxCapsuleContactPerformanceTests, PerformanceTests)
{
	...
	const int numberOfTests = 22;
	std::array<SurgSim::Framework::Timer, numberOfTests> timers;
	std::array<std::string, numberOfTests> tests;
	...
	for (int i = 0; i < numberOfLoops; ++i)
	{
		int timerCount = 0;
		{
			tests[timerCount] = "No intersection, box in front of capsule";
			SCOPED_TRACE(tests[timerCount]);
			boxQuat = Quaterniond::Identity();
			boxTrans = Vector3d(10.6, 0.0, 0.0);
			capsuleQuat = Quaterniond::Identity();
			capsuleTrans = Vector3d::Zero();
			doBoxCapsuleTest(box, boxQuat, boxTrans, capsule, capsuleQuat, capsuleTrans, &timers[timerCount++]);
		}

		{
			tests[timerCount] = "No intersection, capsule beyond corner of box";
			boxQuat = Quaterniond::Identity();
		...
		{
			tests[timerCount] = "No intersection, box below capsule";
			boxQuat = Quaterniond::Identity();
		...
		{
			tests[timerCount] = "Intersection, box intersection with capsule side";
			boxQuat = Quaterniond::Identity();
		...
		{
			tests[timerCount] = "Intersection, box intersection with upside down capsule";
			boxQuat = Quaterniond::Identity();
			boxTrans = Vector3d(1.0 , 0.0, 0.0);
		...
		{
			tests[timerCount] = "Intersection, box intersection with z-axis capsule";
			boxQuat = Quaterniond::Identity();
			boxTrans = Vector3d(1.0 , 0.0, 0.0);
		...
		{
			tests[timerCount] = "Intersection, box intersection with x-axis capsule";
		...
		{
			tests[timerCount] = "Intersection, box intersection with capsule cap";
		...
		{
			tests[timerCount] = "No intersection, capsule near box corner, but not intersecting";
			boxQuat = Quaterniond::Identity();
		...
		{
			tests[timerCount] = "Intersection, capsule intersecting with box corner";
			boxQuat = Quaterniond::Identity();
			boxTrans = Vector3d(0.0 , 0.0, 0.0);
			capsuleQuat = Quaterniond::Identity();
		...
		{
			tests[timerCount] = "Intersection, box inside capsule";
			boxQuat = Quaterniond::Identity();
			boxTrans = Vector3d::Zero();
			capsuleQuat = Quaterniond::Identity();
			capsuleTrans = Vector3d::Zero();
		...
		{
			tests[timerCount] = "Intersection, capsule inside box";
			...
			doBoxCapsuleTest(bigBox, boxQuat, boxTrans, capsule, capsuleQuat, capsuleTrans, &timers[timerCount++]);
		}

		{
			tests[timerCount] = "Intersection, capsule bottom at box center";
			boxQuat = Quaterniond::Identity();
			boxTrans = Vector3d::Zero();
			capsuleQuat = Quaterniond::Identity();
		...
		{
			tests[timerCount] = "Intersection, capsule top at box center";
			boxQuat = Quaterniond::Identity();
			boxTrans = Vector3d::Zero();
			capsuleQuat = Quaterniond::Identity();
			capsuleTrans = Vector3d(0.0, 2.0, 0.0);
		...
		{
			tests[timerCount] = "Intersection with box edge, box's point on edge";
			boxQuat = Quaterniond::Identity();
			boxTrans = Vector3d::Zero();
		...
		{
			tests[timerCount] =
				"Intersection with box edge, box's point on edge, capsule point along vector towards box point";
		...
		{
			tests[timerCount] = "Intersection with box corner";
		...
		{
			tests[timerCount] = "Another intersection with box corner";
		...
		{
			tests[timerCount] = "Intersection with box face, but closest point to box center is outside dilated box";
			std::shared_ptr<BoxShape> box =
	...
	const int nanosecondsInASecond = 1000000000;
	for (size_t i = 0; i < timers.size(); ++i)
	{
		RecordProperty(tests[i] + " (ns)", static_cast<int>(timers[i].getAverageFramePeriod() * nanosecondsInASecond));
	}
}
