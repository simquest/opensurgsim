...
void doGjkCollisionAlgorithmTest(std::shared_ptr<SurgSim::Math::Shape> meshA,
                                const RigidTransform3d& meshATransform,
                                std::shared_ptr<SurgSim::Math::Shape> meshB,
                                const RigidTransform3d& meshBTransform,
                                std::shared_ptr<Contact> expectedContact)
{
    std::shared_ptr<CollisionRepresentation> meshARep = std::make_shared<MockCollisionRepresentation>(
        "Collision Mesh 0",
        meshA,
        meshATransform);
    std::shared_ptr<CollisionRepresentation> meshBRep = std::make_shared<MockCollisionRepresentation>(
        "Collision Mesh 1",
        meshB,
        meshBTransform);
    ...
    GjkCollisionAlgorithm gjkCollisionAlgorithm;
	
	EXPECT_EQ(expectedContact != nullptr, gjkCollisionAlgorithm.detectCollision(pair));
}


TEST(GjkCollisionAlgorithmTests, UnitTests)
{
...

    {
        ...
        doGjkCollisionAlgorithmTest((std::shared_ptr<Shape>) cubeMeshA,
                                   cubeMeshATransform,
                                   (std::shared_ptr<Shape>) cubeMeshB,
                                   cubeMeshBTransform,
                                   nullptr);
    }
...

    {
        ...
        std::shared_ptr<Contact> expectedContact = std::make_shared<Contact>(expectedDepth,
            Vector3d::Zero(), globalTransform.rotation() * -Vector3d::UnitY(), penetrationPoint);
        doGjkCollisionAlgorithmTest((std::shared_ptr<Shape>) cubeMeshA,
                                   cubeMeshATransform,
                                   (std::shared_ptr<Shape>) cubeMeshB,
                                   cubeMeshBTransform,
...

    {
        ...
        cubeMeshBTransform = globalTransform * cubeMeshBTransform;
        std::shared_ptr<Contact> expectedContact = std::make_shared<Contact>(expectedDepth,
            Vector3d::Zero(), globalTransform.rotation() * Vector3d::UnitX(), penetrationPoint);
        doGjkCollisionAlgorithmTest((std::shared_ptr<Shape>) cubeMeshA,
                                   cubeMeshATransform,
                                   (std::shared_ptr<Shape>) cubeMeshB,
                                   cubeMeshBTransform,
...

    {
        ...
        cubeMeshBTransform = globalTransform * cubeMeshBTransform;
        std::shared_ptr<Contact> expectedContact = std::make_shared<Contact>(expectedDepth,
            Vector3d::Zero(), globalTransform.rotation() * -Vector3d::UnitZ(), penetrationPoint);
        doGjkCollisionAlgorithmTest((std::shared_ptr<Shape>) cubeMeshA,
                                   cubeMeshATransform,
                                   (std::shared_ptr<Shape>) cubeMeshB,
                                   cubeMeshBTransform,
