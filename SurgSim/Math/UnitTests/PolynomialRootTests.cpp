...
TEST_F(PolynomialRootsTest, PolnomialDegree1Roots)
{
	...
	PolynomialRoots<1, double>degenerateRoots(degenerate);
	...
	PolynomialRoots<1, double>constantPolyRoots(constantPoly);
	...
	PolynomialRoots<1, double>linearPolyRoots(linearPoly);
...
TEST_F(PolynomialRootsTest, PolnomialDegree2Roots)
{
	...
	PolynomialRoots<2, double>degenerateRoots(degenerate);
	EXPECT_TRUE(degenerateRoots.isDegenerate());
	EXPECT_EQ(-1, degenerateRoots.getNumRoots());
	EXPECT_THROW(degenerateRoots[0], SurgSim::Framework::AssertionFailure);
	...
	PolynomialRoots<2, double>constantPolyRoots(constantPoly);
	EXPECT_FALSE(constantPolyRoots.isDegenerate());
	EXPECT_EQ(0, constantPolyRoots.getNumRoots());
	EXPECT_THROW(constantPolyRoots[0], SurgSim::Framework::AssertionFailure);
	...
	PolynomialRoots<2, double>linearPolyRoots(linearPoly);
	EXPECT_FALSE(linearPolyRoots.isDegenerate());
	EXPECT_EQ(1, linearPolyRoots.getNumRoots());
	EXPECT_NEAR(12.0, linearPolyRoots[0], epsilon);
	EXPECT_THROW(linearPolyRoots[1], SurgSim::Framework::AssertionFailure);
	...
	PolynomialRoots<2, double>imaginaryPolyRoots(imaginaryPoly);
	...
	PolynomialRoots<2, double>oneRootPolyRoots(oneRootPoly);
	...
	PolynomialRoots<2, double>quadraticPolyRoots(quadraticPoly);
...
TEST_F(PolynomialRootsTest, SolveAssertionTests)
{
	...
	EXPECT_THROW(solve(5.0, 5.0, epsilon, &numRoots, &zeroArray), SurgSim::Framework::AssertionFailure);

	// Quadratic, degenerate to linear.
	EXPECT_THROW(solve(0.0, 5.0, 5.0, epsilon, &numRoots, &zeroArray), SurgSim::Framework::AssertionFailure);

	// Quadratic, duplicate root.
	EXPECT_THROW(solve(1.0, -24.0, 144.0, epsilon, &numRoots, &zeroArray), SurgSim::Framework::AssertionFailure);

	// Quadratic, two roots.
	EXPECT_THROW(solve(2.0, 6.0, -8.0, epsilon, &numRoots, &oneArray), SurgSim::Framework::AssertionFailure);
};
