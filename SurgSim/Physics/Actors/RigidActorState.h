...
/// The RigidActorState class describes a dynamic rigid body state
/// (position + velocity information)
/// Derives from RigidActorBaseState which describes the position state only
class RigidActorState : public RigidActorBaseState
{
public:
	/// Default constructor
	RigidActorState()
	...
	/// Destructor
	virtual ~RigidActorState()
	{
	}

	/// Comparison operator
	/// \param s A RigidActorState to compare it to
	/// \return True if the 2 states are equals, False otherwise
	bool operator ==(const RigidActorState &s) const
	{
		return (RigidActorBaseState::operator ==(s) && m_v == s.m_v && m_w == s.m_w);
	}
	...
	/// in another RigidActorState and assign it to this instance.
	virtual void reset()
	{
