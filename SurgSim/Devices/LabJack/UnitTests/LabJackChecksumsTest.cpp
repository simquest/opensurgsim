...
using SurgSim::Device::LabJackChecksums;
...
TEST(LabJackChecksumsTest, NormalChecksum)
{
	...
	EXPECT_EQ(expectedValue, LabJackChecksums::normalChecksum8(bytes, count));
	LabJackChecksums::normalChecksum(&bytes, count);
...
TEST(LabJackChecksumsTest, ExtendedChecksum)
{
	...
	EXPECT_EQ(expectedValue16, LabJackChecksums::extendedChecksum16(bytes, count));
	int expectedValue8 = (6 - 1) * fillValue; // 5 * 2 = 10
	EXPECT_EQ(expectedValue8, LabJackChecksums::extendedChecksum8(bytes));

	LabJackChecksums::extendedChecksum(&bytes, count);
