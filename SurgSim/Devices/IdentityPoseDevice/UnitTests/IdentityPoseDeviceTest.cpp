...
TEST(IdentityPoseDeviceTest, AddInputConsumer)
{
	...
	EXPECT_TRUE(consumer->m_lastReceivedInput.booleans().hasData(SurgSim::DataStructures::DataNames::button0));

	// Check the data.
	RigidTransform3d pose;
	EXPECT_TRUE(consumer->m_lastReceivedInput.poses().get(SurgSim::DataStructures::DataNames::pose, &pose));
	EXPECT_NEAR(0, (pose.matrix() - Matrix44d::Identity()).norm(), 1e-6);
	bool button0 = false;
	EXPECT_TRUE(consumer->m_lastReceivedInput.booleans().get(SurgSim::DataStructures::DataNames::button0, &button0));
