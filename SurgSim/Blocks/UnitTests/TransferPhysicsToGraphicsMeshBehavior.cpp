...
TEST(TransferPhysicsToGraphicsMeshBehaviorTests, UpdateTest)
{
	...
	SURGSIM_ASSERT(!path.empty()) << "Can not locate file Data/Geometry/wound_deformable.ply.";

	auto physics = std::make_shared<Fem3DRepresentation>("Fem3D");
	physics->setFilename(path);
	physics->loadFile();
...
TEST(TransferPhysicsToGraphicsMeshBehaviorTests, SerializationTest)
{
	...
	std::shared_ptr<SurgSim::Physics::Representation> physics = std::make_shared<Fem3DRepresentation>("Fem3D");
	auto fem3d = std::dynamic_pointer_cast<Fem3DRepresentation>(physics);
	fem3d->setFilename(filename);
	fem3d->loadFile();

	std::shared_ptr<SurgSim::Graphics::Representation> graphics =
	...
	EXPECT_NE(nullptr, newBehavior->getValue<std::shared_ptr<SurgSim::Physics::Representation>>("Source"));
	EXPECT_NE(nullptr, newBehavior->getValue<std::shared_ptr<SurgSim::Graphics::Representation>>("Target"));
}
