...
void MovingSquareForce::updateTool(const DataGroup& inputData)
{
	...
	// Safely update square data in thread-safe storage if the other device updated them
	m_square.update();
...
Vector3d MovingSquareForce::computeForce(const Vector3d& position)
{
	...
	Vector3d offset = position - m_square->center;
	double orthogonalDistance = offset.dot(m_square->normal);

	if ((orthogonalDistance * m_squareNormalDirection) > 0)
	{
		double planeDistanceX = offset.dot(m_square->edgeDirectionX);
		double planeDistanceY = offset.dot(m_square->edgeDirectionY);
		bool insideSquare = (fabs(planeDistanceX) < m_squareHalfSize) && (fabs(planeDistanceY) < m_squareHalfSize);

		force = -m_surfaceStiffness * orthogonalDistance * m_square->normal;
