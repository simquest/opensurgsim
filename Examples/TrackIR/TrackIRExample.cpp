...
std::shared_ptr<SceneElement> createBox(const std::string& name)
{
	...
	std::shared_ptr<BoxShape> box = std::make_shared<BoxShape>(8, 2.0, 2); // in m
	...
	std::shared_ptr<SurgSim::Input::InputComponent> inputComponent =
		std::make_shared<SurgSim::Input::InputComponent>("input", "TrackIRDevice");
...
int main(int argc, char* argv[])
{
	...
	graphicsManager->getDefaultCamera()->setInitialPose(
		SurgSim::Math::makeRigidTransform(SurgSim::Math::Quaterniond::Identity(), Vector3d(0.0, 0.5, 0.0)));
